 ...
 *   and sending JSON responses via the Spring Web MVC framework.
 * - RESTful API Design Pattern: Provides stateless HTTP resource endpoints (GET, POST, DELETE, PATCH).
 * - Global Exception Handling: Note that this class does not contain explicit try-catch blocks for 
 *   business exceptions. Exceptions thrown by the Service layer are automatically intercepted and 
 *   handled by the @RestControllerAdvice configured in GlobalExceptionHandler.
 */
@RestController
public class SocialMediaController {
     ...
     * Instantiates necessary service dependencies via Spring Autowiring.
     * 
     ...
     * @return ResponseEntity with the created Account and status 200 on success, 409 on conflict, 400 on Client error.
     */
     ...
     * 
     * @param account The Account object constructed from the JSON payload.
     * @return ResponseEntity with the authenticated Account and status 200 on success, 401 on Unauthorized.
     */
    @PostMapping("/login")
     ...
     * @return ResponseEntity containing 1 (rows updated) on success with status 200, 400 on Client error.
     */
